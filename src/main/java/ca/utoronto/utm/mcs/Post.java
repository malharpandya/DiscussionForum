...
public class Post implements HttpHandler {
	...
	private Document createPost(String title, String author, String content, ArrayList<String> tags) {
		Document doc = new Document().append("title", title).append("author", author).append("content", content)
				.append("tags", tags);
		return doc;
	}
	...
	private void handleGet(HttpExchange exchange) throws IOException, JSONException {
		...
		try {
			...
			if (deserialized.has("_id")) {
				...
				Iterator<Document> posts = collection.find(text("\"" + search + "\"")).sort(Sorts.ascending("title"))
						.iterator();
				if (!posts.hasNext()) {
